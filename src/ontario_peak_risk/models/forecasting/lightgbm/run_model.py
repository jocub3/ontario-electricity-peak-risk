"""Train, tune, evaluate, and interpret LGBMRegressor."""
...
def _select_best_parameters(
    ...
    grouping = ['parameter_id', 'n_estimators', 'num_leaves', 'learning_rate', 'max_depth', 'min_child_samples', 'subsample', 'colsample_bytree', 'reg_lambda']
...
def run_model(
    config_path: str | Path = "configs/lightgbm_regressor.yaml",
) -> dict[str, object]:
    ...
    if bool(cfg.get("evaluate_final_holdout", False)):
        raise ValueError(
            "LGBMRegressor candidate must not evaluate the final 2025 holdout."
        )
    ...
    save_csv(
        feature_report,
        paths["reports_dir"] / "LGBM_02_feature_dictionary.csv",
    )
    ...
    save_csv(
        tuning_results,
        paths["reports_dir"] / "LGBM_04_tuning_details.csv",
    )
    save_csv(
        tuning_summary,
        paths["reports_dir"] / "LGBM_04_tuning_summary.csv",
    )
    ...
    save_csv(fold_metrics, paths["reports_dir"] / "LGBM_05_fold_metrics.csv")
    save_csv(fsa_metrics, paths["reports_dir"] / "LGBM_05_fsa_metrics.csv")
    save_csv(
        horizon_metrics,
        paths["reports_dir"] / "LGBM_05_horizon_metrics.csv",
    )
    save_csv(
        global_metrics,
        paths["reports_dir"] / "LGBM_05_global_metrics.csv",
    )
    save_csv(
        importance_summary,
        paths["reports_dir"] / "LGBM_06_feature_importance.csv",
    )

    predictions.to_parquet(
        paths["outputs_dir"] / "lightgbm_validation_predictions.parquet",
        index=False,
    )

    print("LGBMRegressor completed successfully.")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run LGBMRegressor candidate model."
    )
    parser.add_argument("--config", default="configs/lightgbm_regressor.yaml")
