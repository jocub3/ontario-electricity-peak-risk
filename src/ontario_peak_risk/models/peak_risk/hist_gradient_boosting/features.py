"""Leakage-safe model-specific features for 24-hour candidate models."""
...
def _season_from_month(month: pd.Series) -> pd.Series:
    """Create meteorological seasons from calendar month."""
...
def _rolling_origin_features(
    ...
    """
    Build safe historical summaries available at each forecast origin.

    Windows include the current forecast-origin observation and never include
    future observations.
    """
...
def build_horizon_features(
    ...
    """
    Build leakage-safe predictors for one forecast horizon.

    Predictors include:
    - FSA and deterministic target-time calendar information;
    - historical demand at the target hour 24h, 48h, and 168h earlier;
    - recent rolling demand summaries available at forecast origin;
    - current weather variables available at forecast origin;
    - current reported premise count, when available.

    No observed future weather or future electricity demand is used.
    """
    ...
    rolling_lookup = _rolling_origin_features(
        source,
...
def model_feature_columns(
    ...
    """Return numeric and categorical predictors available in the dataset."""
    numeric = [
        "target_hour",
        "target_weekday",
        "target_month",
...
def feature_dictionary(
    ...
    """Document the candidate model predictor set."""
