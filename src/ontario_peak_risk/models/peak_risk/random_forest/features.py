"""Leakage-safe direct-horizon features for Peak-Risk tree classifiers."""
...
def _origin_rolling_lookup(
    ...
    work = (
        source[[group_column, time_column, demand_column]]
        .sort_values([group_column, time_column], kind="stable")
        .copy()
    )

    grouped = work.groupby(group_column, observed=True)[demand_column]

    # Shift by one hour before rolling so the current hour's demand is excluded.
    ...
    work["origin_rolling_mean_168h"] = grouped.transform(
        lambda s: s.shift(1).rolling(168, min_periods=24).mean()
    )
...
def build_horizon_features(
    ...
    *,
    ...
    """
    Build one predictor matrix for a direct Peak-Risk horizon.

    Predictors use only:
    - deterministic target-time calendar information;
    - historical demand values available at forecast origin;
    - weather/current context observed at forecast origin.

    Future observed weather and future observed demand are never used.
    """
    horizon = int(horizon)
    if not 1 <= horizon <= 24:
    ...
    rolling_lookup = _origin_rolling_lookup(
        source,
    ...
    # target_lag_24h means demand at target_time - 24h.
    # For horizon <= 24 this timestamp is never later than forecast_origin.
