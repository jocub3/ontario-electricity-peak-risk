"""Train, tune, evaluate, and interpret XGBClassifier for Peak-Risk."""
...
def run_model(
    config_path: str | Path = "configs/xgboost_classifier.yaml",
    *,
    resume: bool = True,
) -> dict[str, object]:
    ...
    save_csv(
        feature_report,
        paths["reports_dir"] / "XGBC_02_feature_dictionary.csv",
    )

    print("=" * 72, flush=True)
    print("XGBClassifier — PEAK-RISK CANDIDATE", flush=True)
    ...
    save_csv(
        tuning_results,
        paths["reports_dir"] / "XGBC_04_tuning_details.csv",
    )

    save_csv(
        tuning_summary,
        paths["reports_dir"] / "XGBC_04_tuning_summary.csv",
    )
    ...
    save_csv(
        ...
        paths["reports_dir"] / "XGBC_05_global_metrics.csv",
    )
    save_csv(
        grouped["fold_metrics"],
        paths["reports_dir"] / "XGBC_05_fold_metrics.csv",
    )
    save_csv(
        grouped["fsa_metrics"],
        paths["reports_dir"] / "XGBC_05_fsa_metrics.csv",
    )
    save_csv(
        grouped["horizon_metrics"],
        paths["reports_dir"] / "XGBC_05_horizon_metrics.csv",
    )
    save_csv(
        grouped["class_balance"],
        paths["reports_dir"] / "XGBC_05_class_balance.csv",
    )
    save_csv(
        threshold_analysis,
        paths["reports_dir"] / "XGBC_05_threshold_analysis.csv",
    )
    save_csv(
        importance_summary,
        paths["reports_dir"] / "XGBC_06_feature_importance.csv",
    )

    predictions.to_parquet(
        paths["outputs_dir"]
        / "xgboost_classifier_validation_predictions.parquet",
        index=False,
    )

    print("=" * 72, flush=True)
    print("XGBClassifier completed successfully.", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run XGBClassifier Peak-Risk candidate."
    )
    parser.add_argument(
        "--config",
        default="configs/xgboost_classifier.yaml",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
