"""Train, tune, evaluate, and interpret LGBMClassifier for Peak-Risk."""
...
def _build_pipeline(
    ...
    *,
    ...
    scale_pos_weight = (
        float(class_ratio)
        * float(parameters.get("imbalance_multiplier", 1.0))
    )
...
def run_model(
    config_path: str | Path = "configs/lightgbm_classifier.yaml",
    *,
    resume: bool = True,
) -> dict[str, object]:
    ...
    save_csv(
        feature_report,
        paths["reports_dir"] / "LGBC_02_feature_dictionary.csv",
    )

    print("=" * 72, flush=True)
    print("LGBMClassifier — PEAK-RISK CANDIDATE", flush=True)
    ...
    save_csv(
        tuning_results,
        paths["reports_dir"] / "LGBC_04_tuning_details.csv",
    )

    save_csv(
        tuning_summary,
        paths["reports_dir"] / "LGBC_04_tuning_summary.csv",
    )
    ...
    save_csv(
        ...
        paths["reports_dir"] / "LGBC_05_global_metrics.csv",
    )
    save_csv(
        grouped["fold_metrics"],
        paths["reports_dir"] / "LGBC_05_fold_metrics.csv",
    )
    save_csv(
        grouped["fsa_metrics"],
        paths["reports_dir"] / "LGBC_05_fsa_metrics.csv",
    )
    save_csv(
        grouped["horizon_metrics"],
        paths["reports_dir"] / "LGBC_05_horizon_metrics.csv",
    )
    save_csv(
        grouped["class_balance"],
        paths["reports_dir"] / "LGBC_05_class_balance.csv",
    )
    save_csv(
        threshold_analysis,
        paths["reports_dir"] / "LGBC_05_threshold_analysis.csv",
    )
    save_csv(
        importance_summary,
        paths["reports_dir"] / "LGBC_06_feature_importance.csv",
    )

    predictions.to_parquet(
        paths["outputs_dir"]
        / "lightgbm_classifier_validation_predictions.parquet",
        index=False,
    )

    print("=" * 72, flush=True)
    print("LGBMClassifier completed successfully.", flush=True)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run LGBMClassifier Peak-Risk candidate."
    )
    parser.add_argument(
        "--config",
        default="configs/lightgbm_classifier.yaml",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
