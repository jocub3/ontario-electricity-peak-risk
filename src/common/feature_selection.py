...
said `hour_sin`/`hour_cos` were useful (they weren't) and said `Dew Point Temp (°C)` wasn't (it was).
...
only span that's never a test year in any fold, so it's the only safe choice for this kind of check.
...
information by construction, unlike a correlated-but-distinct measurement), so this isn't surprising
...
they're noise here, not just low-gain.
...
was small and inconsistent in direction across the two checks, but it's a single column, the EDA
